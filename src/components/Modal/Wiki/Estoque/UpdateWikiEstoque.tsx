...
import {
  Container,
  Wrapper
} from '../../../../styles/components/Modal/Wiki/Estoque/UpdateWikiEstoque'

interface UpdateWikiEstoqueProps {
  id: string
...
export function UpdateWikiEstoque({
  id,
  faqs,
  setFaqs,
  selectedFaq
}: UpdateWikiEstoqueProps) {
  ...
  const handleSubmit = useCallback(
    async (data, { reset }) => {
      ...
      try {
        ...
        const remainingFaqsEstoque = faqs.filter(
          faq => faq.wiki_id !== selectedFaq.wiki_id
        )

        setFaqs([response.data.wiki, ...remainingFaqsEstoque])
  ...
  return (
    <GlobalModal size={600} title="Editar FAQ Estoque" id={id}>
      <Container
        initialData={{
      ...
      >
        ...
        <section>
          ...
          {permissions?.includes('WIKI.TIPOS.DELETAR') && (
            <Button
              onClick={() =>
                setDisplayModal([...displayModal, 'modalDeleteWikiEstoque'])
              }
