...
import { WikiRelatorioProps } from '../../../../pages/wiki/relatorio'
...
import {
  Container,
  Wrapper
} from '../../../../styles/components/Modal/Wiki/Relatorio/UpdateWikiRelatorio'

interface UpdateWikiRelatorioProps {
  id: string
  faqs: WikiRelatorioProps[]
  setFaqs: (faqs: WikiRelatorioProps[]) => void
  selectedFaq: WikiRelatorioProps
}

export function UpdateWikiRelatorio({
  id,
  faqs,
  setFaqs,
  selectedFaq
}: UpdateWikiRelatorioProps) {
  ...
  const handleSubmit = useCallback(
    async (data, { reset }) => {
      ...
      try {
        ...
        const remainingFaqsRelatorio = faqs.filter(
          faq => faq.wiki_id !== selectedFaq.wiki_id
        )

        setFaqs([response.data.wiki, ...remainingFaqsRelatorio])
  ...
  return (
    <GlobalModal size={600} title="Editar FAQ Relatório" id={id}>
      <Container
        initialData={{
      ...
      >
        ...
        <section>
          ...
          {permissions?.includes('WIKI.TIPOS.DELETAR') && (
            <Button
              onClick={() =>
                setDisplayModal([...displayModal, 'modalDeleteWikiRelatorio'])
              }
