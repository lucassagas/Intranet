...
import {
  Container,
  Wrapper
} from '../../../../styles/components/Modal/Wiki/Noc/UpdateWikiNoc'

interface UpdateWikiNocProps {
  id: string
  faqs: WikiNocProps[]
...
export function UpdateWikiNoc({
  ...
  const handleSubmit = useCallback(
    async (data, { reset }) => {
      ...
      try {
        ...
        const remainingFaqsNoc = faqs.filter(
          faq => faq.wiki_id !== selectedFaq.wiki_id
        )

        setFaqs([response.data.wiki, ...remainingFaqsNoc])
  ...
  return (
    <GlobalModal size={600} title="Editar FAQ NOC" id={id}>
      <Container
        initialData={{
      ...
      >
        ...
        <section>
          ...
          {permissions?.includes('WIKI.TIPOS.DELETAR') && (
            <Button
              onClick={() =>
                setDisplayModal([...displayModal, 'modalDeleteWikiNoc'])
              }
