...
import {
  Container,
  Wrapper
} from '../../../../styles/components/Modal/Wiki/Recepcao/UpdateWikiRecepcao'

interface UpdateWikiRecepcaoProps {
  id: string
...
export function UpdateWikiRecepcao({
  id,
  faqs,
  setFaqs,
  selectedFaq
}: UpdateWikiRecepcaoProps) {
  ...
  const handleSubmit = useCallback(
    async (data, { reset }) => {
      ...
      try {
        ...
        const remainingFaqsRecepcao = faqs.filter(
          faq => faq.wiki_id !== selectedFaq.wiki_id
        )

        setFaqs([response.data.wiki, ...remainingFaqsRecepcao])
  ...
  return (
    <GlobalModal size={600} title="Editar FAQ Recepção" id={id}>
      <Container
        initialData={{
      ...
      >
        ...
        <section>
          ...
          {permissions?.includes('WIKI.TIPOS.DELETAR') && (
            <Button
              onClick={() =>
                setDisplayModal([...displayModal, 'modalDeleteWikiRecepcao'])
              }
