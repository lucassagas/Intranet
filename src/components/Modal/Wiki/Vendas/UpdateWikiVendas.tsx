...
import {
  Container,
  Wrapper
} from '../../../../styles/components/Modal/Wiki/Vendas/UpdateWikiVendas'

interface UpdateWikiVendasProps {
  id: string
  faqs: WikiVendasProps[]
  setFaqs: (faqs: WikiVendasProps[]) => void
  selectedFaq: WikiVendasProps
}

export function UpdateWikiVendas({
  id,
  faqs,
  setFaqs,
  selectedFaq
}: UpdateWikiVendasProps) {
  ...
  const handleSubmit = useCallback(
    async (data, { reset }) => {
      ...
      try {
        ...
        const remainingFaqsVendas = faqs.filter(
          faq => faq.wiki_id !== selectedFaq.wiki_id
        )

        setFaqs([response.data.wiki, ...remainingFaqsVendas])
  ...
  return (
    <GlobalModal size={600} title="Editar FAQ Vendas" id={id}>
      <Container
        initialData={{
      ...
      >
        ...
        <section>
          ...
          {permissions?.includes('WIKI.TIPOS.DELETAR') && (
            <Button
              onClick={() =>
                setDisplayModal([...displayModal, 'modalDeleteWikiVendas'])
              }
